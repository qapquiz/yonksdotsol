...
export default function PositionLiquidityWidget({
  ...
  // Rendered-height budget (dp, incl. margins) per optional row. The chart claims
  // exactly the space left after the rows it keeps — relying on the `flex: 1`
  // LinearLayout weight instead let short widgets collapse the graph to 0dp
  // (fixed rows overflowed and consumed all leftover space), erasing the chart
  // on ~2-row widgets even though their data was healthy.
  const CHROME = {
    padding: INSET * 2,
    header: 54,
    statusRow: 24,
  ...
  const showFees = height >= 310
  const showHint = showFees
  const showAddress = height >= 260
  const showShapeLabel = height >= 230
  const showPriceLabels = showShapeLabel
  const showValue = height >= 190

  const fixedChrome =
    CHROME.padding +
    CHROME.header +
    CHROME.footer +
    (showAddress ? CHROME.statusRow : 0) +
    (showValue ? CHROME.valueRow : 0) +
    (showShapeLabel ? CHROME.shapeLabel : 0) +
    (showPriceLabels ? CHROME.priceLabels : 0) +
    (showHint ? CHROME.hint : 0) +
    (showFees ? CHROME.feesRow : 0) +
    (message ? 18 : 0)
  const chartHeight = Math.max(32, Math.min(160, height - fixedChrome))
