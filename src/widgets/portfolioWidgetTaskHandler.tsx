...
async function fetchPortfolioSummary(walletAddress: string) {
  ...
  // 2. Count out-of-range positions and collect unique mints
  ...
  for (const position of positionsArray) {
    ...
    // Check in-range status: activeId >= lowerBinId && activeId <= upperBinId
  ...
  // 6. Count total individual positions for display
...
async function portfolioWidgetTaskHandler({ widgetAction, clickAction, renderWidget }: WidgetTaskHandlerProps) {
  ...
  if (!walletAddress) {
    renderWidget(<ErrorWidget message="Connect wallet in app to see portfolio data" />)
    return
  }
