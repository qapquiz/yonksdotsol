...
export function calculateIsInRange(
	currentActiveId: number,
	lowerBinId: number,
	upperBinId: number,
): boolean {
...
export function generateLiquidityChartData(
	...
	for (let i = lowerBinId; i <= upperBinId; i++) {
		...
		const positionXAmountInSOL =
			(Number(bin?.positionXAmount || 0) / 10 ** tokenXDecimal) *
			Number(bin?.pricePerToken || 0);
		const positionYAmountInSOL =
			Number(bin?.positionYAmount || 0) / 10 ** tokenYDecimal;
		chartBins.push({
			binId: i,
...
export function calculatePriceRange(
	...
	const maxPrice =
		chartBins.length > 0
			? positionBinData.length > 0
				? Number(
						positionBinData[positionBinData.length - 1].pricePerToken,
					).toPrecision(6)
				: "0"
			: "0";
...
export function calculatePriceTicks(
	...
	for (let i = 0; i < tickCount; i++) {
		...
		const currentIndex = chartBins.findIndex(
			(b) => b.binId === currentActiveId,
		);
		const isCurrent =
			i ===
			Math.round((currentIndex / (chartBins.length - 1)) * (tickCount - 1));
...
export function calculateActiveBinPosition(
	chartBins: ChartBinData[],
	currentActiveId: number,
): number {
...
export function calculateUPNLValue(
...
export function calculateUPNLPercentage(
	currentTotalValue: number,
	initialDepositValue: number,
): number {
	if (initialDepositValue === 0) return 0;
	return (
		((currentTotalValue - initialDepositValue) / initialDepositValue) * 100
	);
}

export function formatUPNLDisplay(
	upnlValue: number,
	upnlPercentage: number,
): string {
