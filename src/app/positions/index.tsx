...
export default function PositionsList({
	...
	const positionsEntries = useMemo(
		() => Array.from(positions.entries()),
		[positions],
	);
	const positionsArray = useMemo(
		() => positionsEntries.map(([, pos]) => pos),
		[positionsEntries],
	);

	// Extract pool addresses for PnL fetching
	const poolAddresses = useMemo(
		() => positionsEntries.map(([pairAddress]) => pairAddress),
		[positionsEntries],
	);
					...
					return (
						<View className="flex-row items-center gap-2 mb-4 px-1">
							...
							<Text className="text-orange-400 text-xs font-sans-bold">
								{item.count} {item.count === 1 ? "position" : "positions"} out
								of range
							</Text>
						</View>
					);
	...
	const showSkeleton =
		!walletResolved || (positionsArray.length === 0 && isLoadingPositions);
	const showEmpty =
		walletResolved && !isLoadingPositions && positionsArray.length === 0;
