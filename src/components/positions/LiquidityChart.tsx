...
function LiquidityChartComponent({
	...
	const maxLiquidity = liquidityShape
		? Math.max(
				...chartBins.map(
		...
		: 0;
	const minPrice =
		chartBins.length > 0 ? chartBins[0].price.toPrecision(6) : "0";
	const maxPrice =
		chartBins.length > 0
			? chartBins[chartBins.length - 1].price.toPrecision(6)
			: "0";
	...
	const resampledData = useMemo(() => {
		...
		if (chartBins.length <= TARGET_WIDTH) {
			return chartBins.map((b: ChartBinData) => ({
				liquidity: b.positionXAmountInSOL + b.positionYAmountInSOL,
		...
		for (let i = 0; i < TARGET_WIDTH; i++) {
			...
			const maxSliceLiquidity = Math.max(
				...slice.map(
					(b: ChartBinData) => b.positionXAmountInSOL + b.positionYAmountInSOL,
				),
			);

			// If the bucket contains the active bin, mark the whole char as active
			const hasActive = slice.some(
				(b: ChartBinData) => b.binId === currentActiveId,
			);

			// If all bins in bucket are to the left, color it left (green)
			const isLeft = slice.every(
				(b: ChartBinData) => b.binId < currentActiveId,
			);
