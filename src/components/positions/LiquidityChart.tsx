...
import type { ChartBinData, LiquidityShape } from '../../utils/positions/calculations'
...
function LiquidityChartComponent({ liquidityShape, currentPrice }: LiquidityChartProps) {
  ...
  const maxLiquidity = liquidityShape
    ? Math.max(...chartBins.map((b) => b.positionXAmountInSOL + b.positionYAmountInSOL))
    : 0
  ...
  const resampledData = useMemo(() => {
    ...
    for (let i = 0; i < TARGET_WIDTH; i++) {
      ...
      const maxSliceLiquidity = Math.max(
        ...slice.map((b: ChartBinData) => b.positionXAmountInSOL + b.positionYAmountInSOL),
      )
