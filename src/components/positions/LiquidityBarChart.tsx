...
function LiquidityBarChartComponent({ liquidityShape, currentPrice }: LiquidityBarChartProps) {
  ...
  const chartData = useMemo(() => {
    ...
    const maxLiquidity = Math.max(...liquidityShape.binDistribution.map((b) => b.tokenXAmount + b.tokenYAmount))
    const currentActiveId = liquidityShape.currentActiveId

    return liquidityShape.binDistribution.map((bin) => {
      const liquidity = bin.tokenXAmount + bin.tokenYAmount
  ...
  const svgContent = useMemo(() => {
    ...
    const bars = chartData.map((bar, index) => {
      ...
      return <Rect key={`bar-${index}`} x={x} y={y} width={actualBarWidth} height={barHeight} fill={bar.color} rx={2} />
    })
