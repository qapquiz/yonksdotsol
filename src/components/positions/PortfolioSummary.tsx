...
function formatSmallValue(
	value: number,
): { leadingText: string; superscript: string; digits: string } | null {
...
function PortfolioSummaryComponent({
	...
	const summary = usePnLStore(
		useShallow(selectPoolPnLSummary(wallet, poolAddresses)),
	);

	// Memoize selector to avoid creating a new function on every render
	const hasPoolDataSelector = useMemo(
		() => selectHasPoolData(wallet, poolAddresses),
			...
			<View className="bg-app-surface rounded-3xl p-5 mb-4 border border-app-border">
				<View className="flex-row items-center gap-2 mb-3">
					...
					<View className="bg-app-surface-highlight rounded-full w-5 h-5 items-center justify-center">
						<Text className="text-app-text-muted text-[10px] font-sans-bold">
							{positionCount}
						</Text>
					</View>
				</View>
	...
	return (
		<View className="bg-app-surface rounded-3xl p-5 mb-4 border border-app-border">
			<View className="flex-row items-center gap-2 mb-3">
				...
				<View className="bg-app-surface-highlight rounded-full w-5 h-5 items-center justify-center">
					<Text className="text-app-text-secondary text-[10px] font-sans-bold">
						{positionCount}
					</Text>
				</View>
			</View>
			...
			<View className="mb-4">
				<View className="flex-row items-baseline">
					{sign && (
						<Text className={`text-2xl font-pixel ${pnlColorClass}`}>
							{sign}
						</Text>
					)}
					<SolValue
						value={Math.abs(totalPnlSol)}
						className={`text-2xl font-pixel ${pnlColorClass}`}
					/>
					<Text
						className={`text-sm font-pixel ${pnlColorClass} ml-0.5 opacity-60`}
					>
						SOL
					</Text>
				</View>
			...
			<View className="flex-row justify-between">
				<View>
					<Text className="text-app-text-muted text-[10px] font-sans-bold tracking-wider mb-1">
						VALUE
					</Text>
					<View className="flex-row items-baseline">
						<SolValue
							value={totalValueSol}
							className="text-app-text text-sm font-pixel"
						/>
						<Text className="text-app-text text-[10px] font-pixel ml-0.5 opacity-60">
							SOL
						</Text>
					</View>
				</View>
